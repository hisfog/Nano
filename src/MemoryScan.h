...
*/
static DWORD GetProcessIdByProcessName(LPCTSTR lpName)
{
    DWORD dwPid = 0;
...
static DWORD GetProcessIdByWindowName(LPCTSTR lpName)
{
    DWORD dwPid = 0;
...
static DWORD GetModuleBaseAddress(DWORD dwPid, LPCTSTR lpName)
{
    DWORD dwBase = 0;
...
static DWORD GetModuleSize(DWORD dwPid, LPCTSTR lpName)
{
    DWORD dwSize = 0;
...
static PVOID ReadMemory(HANDLE hProcess, DWORD dwAddr, LPVOID lpBuffer, DWORD dwSize)
{
    PVOID ret = 0;
    BOOL status = ReadProcessMemory(hProcess, (LPCVOID)(dwAddr), lpBuffer ? lpBuffer : &ret, dwSize, NULL);
    return lpBuffer ? (PVOID)status : ret;
}
static BOOL WriteMemory(HANDLE hProcess, DWORD dwAddr, LPCVOID lpBuffer, DWORD dwSize)
{
...
*/
static BOOLEAN CheckPattern(PBYTE pbBytes, PBYTE pbPattern, UINT uLength, UCHAR bWildcard)
{
...
static DWORD FindPattern(PBYTE pbBuffer, DWORD dwBase, DWORD dwSize, PBYTE pbPattern, UINT uLength,
...
static DWORD ChunkFindPattern(HANDLE hProcess, DWORD dwBase, DWORD dwSize, DWORD dwChunkSize, PBYTE pbPattern,
...
static int GetPropCount(HANDLE hProcess, DWORD dwAddress)
{
    return (int)ReadMemory(hProcess, dwAddress + m_nProps, NULL, sizeof(int));
}
static BOOL GetTableName(HANDLE hProcess, DWORD dwAddress, PVOID pBuffer)
{
    DWORD dwNameAddr;
    return ReadMemory(hProcess, dwAddress + m_pNetTableName, &dwNameAddr, sizeof(DWORD)) &&
        ReadMemory(hProcess, dwNameAddr, pBuffer, 128);
}
static DWORD GetTable(HANDLE hProcess, DWORD dwAddress)
{
    return (DWORD)ReadMemory(hProcess, dwAddress + m_pRecvTable, NULL, sizeof(DWORD));
}
static DWORD GetNextClass(HANDLE hProcess, DWORD dwAddress)
{
...
*/
static DWORD ScanTable(HANDLE hProcess, DWORD dwTableAddr, LPCSTR lpVarName, DWORD dwLevel)
{
...
static DWORD FindNetvar(HANDLE hProcess, DWORD dwStart, LPCSTR lpClassName, LPCSTR lpVarName)
{

...
// short knifeIDs[19] = { WEAPON_KNIFE_BAYONET,
//     WEAPON_KNIFE_FLIP,
//     WEAPON_KNIFE_GUT,
...
// };
UINT SkinManager::GetModelIndexByName(const char* modelName)
{
...
UINT SkinManager::GetModelIndex(const short itemIndex)
{
    UINT ret = 0;
    switch (itemIndex)
    {
...
UINT SkinManager::GetWeaponSkin(const short itemIndex)
{
    ...
    {
    case WEAPON_DEAGLE:
        paint = SKIN_WEAPON_DEAGLE;
        break;
    case WEAPON_GLOCK:
        paint = SKIN_WEAPON_GLOCK;
        break;
    case WEAPON_AK47:
        paint = SKIN_WEAPON_AK47;
        break;
    case WEAPON_AWP:
        paint = SKIN_WEAPON_AWP;
        break;
    case WEAPON_M4A1:
        paint = SKIN_WEAPON_M4A4;
        break;
    case WEAPON_SSG08:
        paint = SKIN_WEAPON_SSG08;
        break;
    case WEAPON_M4A1_SILENCER:
        paint = SKIN_WEAPON_M4A1S;
        break;
    case WEAPON_USP_SILENCER:
        paint = SKIN_WEAPON_USPS;
        break;
    case GLOVE_T_SIDE:
        paint = SKIN_GLOVE_T_SIDE;
        break;
    case GLOVE_CT_SIDE:
        paint = SKIN_GLOVE_CT_SIDE;
        break;
...
UINT SkinManager::LoadSkins(const char* file, char*** names, UINT** values)
{
    FILE* fp;
    UINT i = 0;
...
void SkinManager::SortSkins(UINT count, char*** names, UINT** values)
{
    UINT vtmp = 0;
    char* ntmp = NULL;
...
void SkinManager::PrintMenu(const char* title, char** name, UINT sz, UINT x)
{
    printf("%s %c %s %c\t\t\t\r", title, x > 0 ? '<' : '|', name[x], x < sz ? '>' : '|');
    Sleep(sz < 20 ? 150 : 35);
}

// UINT x_index = 0; // index of current item
// UINT x_index_model = 0; // index of current item

// UINT ItemSelect(const char* title, char** name, UINT sz)
// {
//     PrintMenu(title, name, sz, x_index);

//     while (!GetAsyncKeyState(20))
//     {
//         if (GetAsyncKeyState(VK_RIGHT) && x_index < sz)
//         {
//             PrintMenu(title, name, sz, ++x_index);
//             while(GetAsyncKeyState(VK_RIGHT));
//         }
//         else if (GetAsyncKeyState(VK_LEFT) && x_index > 0)
//         {
//             PrintMenu(title, name, sz, --x_index);
...
//     return x_index;
// }
// UINT ItemSelectKnifeModel(const char* title, char** name, UINT sz)
// {
//     PrintMenu(title, name, sz, x_index_model);

//     while (!GetAsyncKeyState(20))
//     {
//         if (GetAsyncKeyState(VK_RIGHT) && x_index_model < sz)
//         {
//             PrintMenu(title, name, sz, ++x_index_model);
//             while(GetAsyncKeyState(VK_RIGHT));
//         }
//         else if (GetAsyncKeyState(VK_LEFT) && x_index_model > 0)
//         {
//             PrintMenu(title, name, sz, --x_index_model);
//             while(GetAsyncKeyState(VK_LEFT));
...
//     printf("%s %s\t\t\t\n", title, name[x_index_model]);
//     Sleep(50);
//     return x_index_model;
// }

void SkinManager::xSkins(const short knifeIndex, const UINT knifeSkin)
{
    ...
    while (!GetAsyncKeyState(VK_DELETE))
    {
        ...
        for (UINT i = 0; i < 8; i++)
        {
            ...
            UINT weaponSkin = GetWeaponSkin(weaponIndex);//judge the type of weapon and choose the skin correspounding.
            ...
            if (weaponSkin) // set skin properties, for guns and knives both
            {
                WriteMemory(hProcess, currentWeapon + m_iItemIDHigh, &itemIDHigh, sizeof(int));//the name 
                WriteMemory(hProcess, currentWeapon + m_nFallbackPaintKit, &weaponSkin, sizeof(UINT));//the skin type
                WriteMemory(hProcess, currentWeapon + m_flFallbackWear, &fallbackWear, sizeof(float));//the wear value
            }
        }
...
int SkinManager::SkinCheat()
{
    ...
    // {
    //     printf("[error] Error loading skins from file skins.txt!\n");
    //     return 1;
    // }

    // printf("[+] Loaded %d skins from file\n", count);
    //SortSkins(count, &skinNames, &skinIDs);
    ifstream fileSkinConfig("C:\\Users\\Administrator\\Desktop\\GProjects\\BestHack\\recent\\Release\\SkinConfig.ini"); 
    // ifstream fileSkinConfig("./SkinConfig.ini"); 
    //ofstream outfile("G:\\C++ project\\Read\\out.txt", ios::app); 
    string temp;
    regex pattern("([a-zA-Z_0-9]+)\\s=\\s([\\d]+)"); 
    smatch matchs;
    if (!fileSkinConfig.is_open()) 
    { 
        cout << "[error] can not open file SkinConfig.ini" << endl; 
        return 0;
    } 
    map<string,int> flaps;
    while(getline(fileSkinConfig,temp)) 
    { 
        if(regex_search(temp, matchs, pattern))
        {
            int GetValue=str2int(matchs.str(2));
            flaps[matchs.str(1)]=str2int(matchs.str(2));
        }
    } 
    fileSkinConfig.close(); 

    UINT knifeID = flaps["KNIFE_MODEL"];
    UINT skinID = flaps["KNIFE_SKIN"];
    SKIN_KNIFE_MODEL = flaps["KNIFE_MODEL"];
    SKIN_KNIFE_SKIN = flaps["KNIFE_SKIN"];
    SKIN_WEAPON_AK47 = flaps["WEAPON_AK47"];
    SKIN_WEAPON_M4A4 = flaps["WEAPON_M4A4"];
    SKIN_WEAPON_M4A1S = flaps["WEAPON_M4A1S"];
    SKIN_WEAPON_DEAGLE = flaps["WEAPON_DEAGLE"];
    SKIN_WEAPON_GLOCK =flaps["WEAPON_GLOCK"];
    SKIN_WEAPON_USPS = flaps["WEAPON_USPS"];
    SKIN_WEAPON_AWP = flaps["WEAPON_AWP"];
    SKIN_WEAPON_SSG08 = flaps["WEAPON_SSG08"];

    // UINT knifeID = ItemSelectKnifeModel("[+] Knife model:", knifeNames, sizeof(knifeIDs) / sizeof(knifeIDs[0]) - 1);
